...
FROM alpine:latest AS catalogue_of_life_dl

ARG COL_VERSION="2025-05-13"
ENV COL_VERSION=${COL_VERSION}
ADD https://download.checklistbank.org/col/monthly/${COL_VERSION}_dwca.zip /workspace/data/catalogue_of_life/${COL_VERSION}_dwca.zip

WORKDIR /workspace/data/catalogue_of_life/
RUN unzip ${COL_VERSION}_dwca.zip Taxon.tsv VernacularName.tsv && rm -f ${COL_VERSION}_dwca.zip

FROM pandas AS catalogue_of_life

COPY --from=catalogue_of_life_dl /workspace/data/catalogue_of_life/ /workspace/data/catalogue_of_life/

WORKDIR /workspace/
ADD parse_dwca.py /workspace/
RUN python3 parse_dwca.py \
    --input /workspace/data/catalogue_of_life/ \
    --output /workspace/resources/catalogue_of_life/ \
    --taxonomic_names \
    --vernacular_names \
    --col
...
COPY --from=catalogue_of_life /workspace/resources/ /app/resources/
